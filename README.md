...
* Release [_37.0.0_](https://github.com/sbabcoc/Selenium-Foundation/releases/tag/v37.0.0) of **Selenium Foundation** revises the local **Selenium Grid** node creation API to associate each **Appium** node with the driver plug-in that launched it, enabling reliable driver identification for grids running more than one **Appium** personality. Local **Grid** server construction also picks up its driver "personalities" directly at instantiation, rather than through a fragile post-construction mutation of a previously-empty collection. Redundant hub port/URL parameters were also removed from several public method signatures.
  * **NOTE**: This is a major-version release because these revisions include breaking changes to public interfaces (**`ManagedDriverPlugin.create`**, **`LocalGridServer`**/**`AppiumGridServer`** constructors).
  * **NOTE**: See the companion [_37.0.0_](https://github.com/sbabcoc/selenium-grid-manager/releases/tag/v37.0.0) release of **selenium-grid-manager**, which migrates **Android** device provisioning from per-session initialization to pre-suite provisioning — the primary motivation for this revision.
* Release [_36.0.0_](https://github.com/sbabcoc/Selenium-Foundation/releases/tag/v36.0.0) of **Selenium Foundation** adds a version-agnostic **Grid API Provider** infrastructure for inspecting **Selenium 3** and **Selenium 4** hubs through a single interface, along with discriminating hub-type detection.
  * **NOTE**: This is a major-version release. Local **Grid** management (hub/node configuration templates, `FoundationSlotMatcher`, and related APIs) has moved to **selenium-grid-manager**, whose companion [_36.0.0_](https://github.com/sbabcoc/selenium-grid-manager/releases/tag/v36.0.0) release introduces a complete **sidecar servlet infrastructure** for lifecycle management and monitoring of local **Selenium Grid** instances, including a web-based management console, background discovery of unmanaged Grid instances, and version-appropriate shutdown strategies (`LifecycleShutdownStrategy`, `PM2ShutdownStrategy`, `PidShutdownStrategy`).
* Release [_35.0.0_](https://github.com/sbabcoc/Selenium-Foundation/releases/tag/v35.0.0) of **Selenium Foundation** extracts all local **Selenium Grid** management functionality into the new [**selenium-grid-manager**](https://github.com/sbabcoc/selenium-grid-manager) project. **Selenium Foundation** now retains its role as a pure automation framework with no runtime dependency on local Grid management; local Grid launch/lifecycle support (browser plug-ins, the `runGrid` Gradle task, and related tooling) is provided by installing **selenium-grid-manager** alongside it.
  * **NOTE**: This is a major-version release with corresponding breaking changes. Integration between the two projects is enabled by the new `GridManagerPlugin` **ServiceLoader** interface, allowing **selenium-grid-manager** to register Grid factories with **Selenium Foundation** without a compile-time dependency in either direction.
* Release [_33.4.0_](https://github.com/sbabcoc/Selenium-Foundation/releases/tag/v33.4.0) of **Selenium Foundation** adds support for declaring the settings of **Appium** servers via vendor-specific configuration files. This makes complex configurations much easier to specify, and it also enables dynamic configurations via **JavaScript**/**TypeScript** declarations.
  * This release also includes comprehensive documentation of [**Selenium Foundation** settings](docs/SeleniumFoundationSettings.md).
...
|:---|:---|
| <pre>&lt;dependency&gt;<br/>&nbsp;&nbsp;&lt;groupId&gt;com.nordstrom.ui-tools&lt;/groupId&gt;<br/>&nbsp;&nbsp;&lt;artifactId&gt;selenium-foundation&lt;/artifactId&gt;<br/>&nbsp;&nbsp;&lt;version&gt;38.0.0-s3&lt;/version&gt;<br/>&lt;/dependency&gt;</pre> | <pre>&lt;dependency&gt;<br/>&nbsp;&nbsp;&lt;groupId&gt;com.nordstrom.ui-tools&lt;/groupId&gt;<br/>&nbsp;&nbsp;&lt;artifactId&gt;selenium-foundation&lt;/artifactId&gt;<br/>&nbsp;&nbsp;&lt;version&gt;38.0.0-s4&lt;/version&gt;<br/>&lt;/dependency&gt;</pre> |

To add a dependency for Gradle:

| Platform | Dependency |
|:---:|:---|
| **Selenium 3** | <pre>dependencies {<br/>&nbsp;&nbsp;compile 'com.nordstrom.ui-tools:selenium-foundation:38.0.0-s3'<br/>}</pre> |
| **Selenium 4** | <pre>dependencies {<br/>&nbsp;&nbsp;compile 'com.nordstrom.ui-tools:selenium-foundation:38.0.0-s4'<br/>}</pre> |
...
| Selenium 3 | Selenium 4 |
|:---|:---|
| <pre>&lt;dependency&gt;<br/>&nbsp;&nbsp;&lt;groupId&gt;com.nordstrom.ui-tools&lt;/groupId&gt;<br/>&nbsp;&nbsp;&lt;artifactId&gt;selenium-grid-manager&lt;/artifactId&gt;<br/>&nbsp;&nbsp;&lt;version&gt;38.0.0-s3&lt;/version&gt;<br/>&nbsp;&nbsp;&lt;scope&gt;test&lt;/scope&gt;<br/>&lt;/dependency&gt;</pre> | <pre>&lt;dependency&gt;<br/>&nbsp;&nbsp;&lt;groupId&gt;com.nordstrom.ui-tools&lt;/groupId&gt;<br/>&nbsp;&nbsp;&lt;artifactId&gt;selenium-grid-manager&lt;/artifactId&gt;<br/>&nbsp;&nbsp;&lt;version&gt;38.0.0-s4&lt;/version&gt;<br/>&nbsp;&nbsp;&lt;scope&gt;test&lt;/scope&gt;<br/>&lt;/dependency&gt;</pre> |

To add a dependency for Gradle:

| Platform | Dependency |
|:---:|:---|
| **Selenium 3** | <pre>dependencies {<br/>&nbsp;&nbsp;testImplementation 'com.nordstrom.ui-tools:selenium-grid-manager:38.0.0-s3'<br/>}</pre> |
| **Selenium 4** | <pre>dependencies {<br/>&nbsp;&nbsp;testImplementation 'com.nordstrom.ui-tools:selenium-grid-manager:38.0.0-s4'<br/>}</pre> |
