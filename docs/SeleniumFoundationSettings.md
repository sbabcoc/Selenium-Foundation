...
| Setting | Default | Description |
|---------|---------|-------------|
| <details><summary>**APPIUM_CONFIG_PATH**</summary>`appium.config.path`</details> | *(none)* | <details><summary>`Appium` server configuration file name or path [Learn more...](https://appium.io/docs/en/2.7/guides/config)</summary>The `Appium` configuration can be JSON, YAML, or JavaScript (for dynamic configuration)</details> |
| <details><summary>**APPIUM_CLI_ARGS**</summary>`appium.cli.args`</details> | *(none)* | <details><summary>Server arguments passed to `Appium` on launch.</summary>May be declared multiple times in `settings.properties`. For complex configurations, consider using **APPIUM_CONFIG** instead.<br>**NOTE**: Specifying `--default-capabilities` here is deprecated — its value is redirected to **APPIUM_DEFAULT_CAPS** instead, with a warning logged. Set **APPIUM_DEFAULT_CAPS** directly going forward.</details> |
| <details><summary>**APPIUM_DEFAULT_CAPS**</summary>`appium.default.caps`</details> | <details><summary>*skip flags enabled*</summary>`{"appium:skipServerInstallation":true,"appium:skipDeviceInitialization":true}`</details> | <details><summary>JSON object passed as `Appium`'s `--default-capabilities` launch argument.</summary>Applied once, when the `Appium` server process starts — not a per-session setting. Capabilities in this object are merged into every session request that doesn't already specify them; a capability in a client's own session request always takes precedence.<br><br>The default value enables `skipServerInstallation` and `skipDeviceInitialization` for every session, which is safe once the target device has been provisioned and eliminates per-session UiAutomator2 install/init overhead.<br><br>**Overriding replaces the default outright; it is not merged with it.** To target a service that requires its own default capabilities (e.g. a device farm needing vendor authentication), set this property to your own JSON object. If the install/init overhead reduction is still wanted, include the skip capabilities in your override value as well.</details> |
