...
usage()
{
  ...
  echo
  echo "    -p <profile>        {selenium3|selenium4}"
...
mvn "-Drevision=${revision}" ${phase} -P${profile} ${browserProfile} "-Dselenium.context.platform=${targetPlatform}" "${seleniumSettings[@]}"
