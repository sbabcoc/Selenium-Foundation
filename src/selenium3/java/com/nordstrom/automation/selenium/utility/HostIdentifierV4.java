...
public class HostIdentifierV4 {
    private static final Logger LOG = Logger.getLogger(HostIdentifierV4.class.getName());

    private static volatile String hostName;
    private static volatile String hostAddress;

    private static String resolveHostName() {
        // Ideally, we'd use InetAddress.getLocalHost, but this does a reverse DNS
        // lookup. On Windows
        // and Linux this is apparently pretty fast, so we don't get random hangs. On OS
        // X it's
            ...
            try {
                ...
                if (process.exitValue() == 0) {
                    try (InputStreamReader isr = new InputStreamReader(process.getInputStream(),
                            Charset.defaultCharset()); BufferedReader reader = new BufferedReader(isr)) {
