...
public class OperaPlugin extends ChromePlugin {
    ...
    static {
        ...
        EXTENSION_PATH = Objects.requireNonNull(SeleniumConfig.getConfigPath("navigate-extension"),
                "Failed finding the 'navigate' extension");
        ...
        PERSONALITIES = Map.of(OperaCaps.DRIVER_NAME, BASELINE
                .replace("<ext-path>", EXTENSION_PATH.replaceAll("\\\\", "/")));
    }
    ...
    public static void foo(WebDriver driver, String url) {
        // Build the message
        String json = "{\"action\":\"openUrl\", \"url\":\"%s\"}".formatted(url);
    
        // Base64 encode it for safe injection
        String encoded = Base64.getEncoder().encodeToString(json.getBytes(StandardCharsets.UTF_8));
    
        // JavaScript to send message to extension
        String script = """
            (async () => {
                const message = JSON.parse(atob('%s'));
